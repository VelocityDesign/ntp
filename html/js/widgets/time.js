  ...
  constructor() {
    ...
    if (settings['display'] == false) {
      ...
      const TimeWidgetTick = () => {
        const timeWidgetWorker = new Date();
        settings = JSON.parse(ls.getItem('timeSettings'));
        const timeFormat = settings['time-format']
          .replace(
            'hh',
            timeWidgetWorker.getHours().toString().padStart(2, '0')
          )
          .replace(
            'mm',
            timeWidgetWorker.getMinutes().toString().padStart(2, '0')
          )
          .replace(
            'ss',
            timeWidgetWorker.getSeconds().toString().padStart(2, '0')
          )
          .replace(
            'ms',
            timeWidgetWorker.getMilliseconds().toString().padStart(2, '0')
          );
        const timeWidgetElementFormat = `
                    ...
                    style='display: block; color: var(--backg-text-color); font-size: 1.5rem;font-weight: 400; opacity: 0.7;'>
                        ${days[timeWidgetWorker.getDay()]}, ${nth} ${
          months[timeWidgetWorker.getMonth()]
        }
                    </time>`;
        document.getElementById(
          'time@dothq.co'
        ).innerHTML = timeWidgetElementFormat;
      };

      var timeWidgetElement = document.createElement('div');
      timeWidgetElement.setAttribute('id', 'time@dothq.co');
      timeWidgetElement.setAttribute(
        'class',
        `widget ${settings['widget-position']}`
      );
      document
        .getElementById('widgets-container')
        .appendChild(timeWidgetElement);
      TimeWidgetTick();
