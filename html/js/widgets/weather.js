class WeatherWidget {
  constructor() {
    ...
    if (settings['display'] == false) {
      ...
      weatherWidgetElement.setAttribute(
        'class',
          ...
          if (response.status == 200) {
            response.json().then((weatherInfomation) => {
              ...
              document.getElementById('weatherWidget').innerHTML = `
                            <img src='/images/weather/cloud.svg'/>
                            <h1 style='color: white; margin: 10px 0px 0px 0px'>${Math.floor(
                              temperature
                            )}°${settings['unit'].toUpperCase()}</h1>
                            <p style='color: white; margin: 10px 0px 0px 0px'>${
                              settings['city']
                            }, ${settings['country']}</p>
                        `;
              document.getElementById('weatherWidget').setAttribute(
                'style',
                `
                            display: flex;
                            align-items: ${settings['text-position']};
